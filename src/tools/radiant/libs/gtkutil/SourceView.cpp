...
namespace gtkutil
{
    ...
    SourceView::SourceView (const std::string& language, bool readOnly)
    {
        ...
        _buffer = GTK_SOURCE_BUFFER(gtk_source_buffer_new_with_language(lang));
        ...
        if (fontDesc != NULL) {
            gtk_widget_override_font(GTK_WIDGET(_view), fontDesc);
